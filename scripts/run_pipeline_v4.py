"""
v4 Pipeline: Class-weighted loss only
- Class-weighted Focal Loss (pixel-frequency weights)
- No oversampling — isolates the effect of re-weighting alone
...
# v4: class weights only, no oversampling (to avoid distribution bias)
...
# ---------- Training ----------
print('\n=== TRAINING (v4: class-weighted loss only) ===')
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    if val_l < best_val_loss:
        best_val_loss = val_l
        torch.save({'epoch':epoch,'model_state_dict':model.state_dict(),'val_loss':val_l,'val_iou':val_i}, str(MODEL_DIR / 'best_model_v4.pth'))
...
plt.tight_layout(); plt.savefig(str(REPORT_DIR / 'training_curves_v4.png'), dpi=150, bbox_inches='tight'); plt.close()

# ---------- Evaluation ----------
print('\n=== EVALUATION ===')
checkpoint = torch.load(str(MODEL_DIR / 'best_model_v4.pth'), map_location=DEVICE)
...
ax.set_xlabel('IoU'); ax.set_title('Per-Class IoU (v4: class-weighted loss)')
for bar, val in zip(bars, mean_class_iou): ax.text(max(val+0.01,0.01), bar.get_y()+bar.get_height()/2, f'{val:.4f}', va='center')
ax.legend(); plt.tight_layout(); plt.savefig(str(REPORT_DIR / 'evaluation_per_class_iou_v4.png'), dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig(str(REPORT_DIR / 'evaluation_qualitative_v4.png'), dpi=150, bbox_inches='tight'); plt.close()
...
ax.set_xlabel('True'); ax.set_ylabel('Predicted'); ax.set_title(f'Impurity Rate Estimation (v4)\nMAE={mae:.4f}, R2={r2:.4f}')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout(); plt.savefig(str(REPORT_DIR / 'impurity_rate_v4.png'), dpi=150, bbox_inches='tight'); plt.close()

# ---------- Export ----------
print('\n=== EXPORT ===')
model.cpu()
torch.jit.script(model).save(str(MODEL_DIR / 'wheat_segmentation_v4.pt'))
print('Exported TorchScript')
dummy = torch.randn(1,3,IMG_SIZE,IMG_SIZE)
try:
    torch.onnx.export(model, dummy, str(MODEL_DIR / 'wheat_segmentation_v4.onnx'),
...
print(f'{"="*60}')
print('v4: Class-weighted FocalLoss only (no oversampling)')
print('  Files: best_model_v4.pth, wheat_segmentation_v4.pt')
