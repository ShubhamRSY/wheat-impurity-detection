...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    if val_l < best_val_loss:
        best_val_loss = val_l
        torch.save({'epoch':epoch,'model_state_dict':model.state_dict(),'val_loss':val_l,'val_iou':val_i}, 'best_model_v4.pth')
...
plt.tight_layout(); plt.savefig('training_curves_v4.png', dpi=150, bbox_inches='tight'); plt.close()
...
ax.legend(); plt.tight_layout(); plt.savefig('evaluation_per_class_iou_v4.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('evaluation_qualitative_v4.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('impurity_rate_v4.png', dpi=150, bbox_inches='tight'); plt.close()

# ---------- Export ----------
print('\n=== EXPORT ===')
model.cpu()
torch.jit.script(model).save('wheat_segmentation_v4.pt')
print('Exported TorchScript')
dummy = torch.randn(1,3,IMG_SIZE,IMG_SIZE)
try:
    torch.onnx.export(model, dummy, 'wheat_segmentation_v4.onnx',
