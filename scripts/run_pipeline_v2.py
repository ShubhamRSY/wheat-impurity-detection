...
plt.tight_layout(); plt.savefig('eda_summary.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('eda_samples.png', dpi=150, bbox_inches='tight'); plt.close()
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    if val_l < best_val_loss:
        best_val_loss = val_l
        torch.save({'epoch':epoch,'model_state_dict':model.state_dict(),'val_loss':val_l,'val_iou':val_i}, 'best_model_advanced.pth')
...
plt.tight_layout(); plt.savefig('training_curves_advanced.png', dpi=150, bbox_inches='tight'); plt.close()

# ---------- Evaluation ----------
print('\n=== EVALUATION ===')
checkpoint = torch.load('best_model_advanced.pth', map_location=DEVICE)
...
ax.legend(); plt.tight_layout(); plt.savefig('evaluation_per_class_iou_advanced.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('evaluation_qualitative_advanced.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('impurity_rate_advanced.png', dpi=150, bbox_inches='tight'); plt.close()

# ---------- Export & Deployment ----------
print('\n=== EXPORT ===')
model.cpu()
torch.jit.script(model).save('wheat_segmentation_advanced.pt')
print('Exported TorchScript')
dummy = torch.randn(1,3,IMG_SIZE,IMG_SIZE)
try:
    torch.onnx.export(model, dummy, 'wheat_segmentation_advanced.onnx',
