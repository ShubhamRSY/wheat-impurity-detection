...
val_ds = WheatDataset(val_ids, IMG_DIR, MASK_DIR, val_transform) if 'WheatDataset' in dir() else None
test_ds = WheatDataset(test_ids, IMG_DIR, MASK_DIR, val_transform) if 'WheatDataset' in dir() else None

# Use the base WheatDataset for val/test (no paste)
class WheatDatasetBase(Dataset):
...
val_ds = WheatDatasetBase(val_ids, IMG_DIR, MASK_DIR, val_transform)
test_ds = WheatDatasetBase(test_ids, IMG_DIR, MASK_DIR, val_transform)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    if val_l < best_val_loss:
        best_val_loss = val_l
        torch.save({'epoch':epoch,'model_state_dict':model.state_dict(),'val_loss':val_l,'val_iou':val_i}, 'best_model_v5.pth')
...
plt.tight_layout(); plt.savefig('training_curves_v5.png', dpi=150, bbox_inches='tight'); plt.close()

# ---------- Evaluation ----------
print('\n=== EVALUATION ===')
checkpoint = torch.load('best_model_v5.pth', map_location=DEVICE)
...
ax.legend(); plt.tight_layout(); plt.savefig('evaluation_per_class_iou_v5.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('evaluation_qualitative_v5.png', dpi=150, bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig('impurity_rate_v5.png', dpi=150, bbox_inches='tight'); plt.close()

# ---------- Export ----------
print('\n=== EXPORT ===')
model.cpu()
torch.jit.script(model).save('wheat_segmentation_v5.pt')
print('Exported TorchScript')
dummy = torch.randn(1,3,IMG_SIZE,IMG_SIZE)
try:
    torch.onnx.export(model, dummy, 'wheat_segmentation_v5.onnx',
